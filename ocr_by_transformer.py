...
class Recognition_Dataset(Dataset):

    def __init__(self, base_data_dir, lbl2id_map, sequence_len, max_ratio, phase='train', pad=0):
        ...
        self.imgs_list = []
        self.lbls_list = []
        with open(self.lbl_path, 'r',encoding='utf-8-sig') as reader:
            for line in reader:
...
class OCR_EncoderDecoder(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask):
        ...
        x = self.decoder(target_embedds, memory, src_mask, tgt_mask)
...
def run_epoch_dist(data_loader, model, loss_compute, device=None):
    ...
    for i, batch in enumerate(data_loader):
        ...
        encode_mask = encode_mask.cuda(non_blocking=True)
        decode_in = decode_in.cuda(non_blocking=True)
        decode_out = decode_out.cuda(non_blocking=True)
        decode_mask = decode_mask.cuda(non_blocking=True)
        ntokens = torch.sum(ntokens).cuda(non_blocking=True)
